package com.ISCAS.OneBeltOneRoad.web.frontend.menu;
...
//        for(BrAnnotationMenuItem brAnnotations : brAnnotationMenuItemList){
////          处理一级目录
...
//                AnnotationMenuData[] annotationMenuDataSub = subAnnotationMenuData(brAnnotations, brAnnotationMenuItemList);
...
//        Integer size = gisMenuService.getSubNumBrFirstLevelAnnotationsMenu();
//      确定权限列表中第一级出现的个数
        Integer size = firstLevelByUserCount(brAnnotationMenuItemList, menuItem);

//      JSON包含的数组
        AnnotationMenuData[] dataItems = new AnnotationMenuData[size];
        int dataItemsIndex = 0;
        for(BrAnnotationMenuItem brAnnotationMenuItem : brAnnotationMenuItemList){
//          处理一级目录
            if(brAnnotationMenuItem.getParentId() == null){
                Integer firstLevelId = brAnnotationMenuItem.getId();
                ...
                for(;menuItemIndex < menuItem.length; menuItemIndex++){
                    if(firstLevelId.equals(Integer.parseInt(menuItem[menuItemIndex]))){
                        AnnotationMenuData dataItem  = new AnnotationMenuData();
                        dataItem.setCaption(brAnnotationMenuItem.getCaption());
                        dataItem.setDescription(brAnnotationMenuItem.getDescription());
                        AnnotationMenuData[] annotationMenuDataSub = subAnnotationMenuDataByUser(brAnnotationMenuItem, brAnnotationMenuItemList, menuItem);
                        dataItem.setItems(annotationMenuDataSub);
                        dataItems[dataItemsIndex++] = dataItem;
                        break;
                    }
    ...
    public Integer firstLevelByUserCount(List<BrAnnotationMenuItem> brAnnotationMenuItemList, String[] menuItem){
        Integer size = 0;
        for(BrAnnotationMenuItem brAnnotationMenuItem : brAnnotationMenuItemList){
            if(brAnnotationMenuItem.getParentId() == null){
                Integer menuItemIndex = 0;
                Integer brAnnotationsId = brAnnotationMenuItem.getId();
                for(;menuItemIndex < menuItem.length; menuItemIndex++){
    ...
    public  AnnotationMenuData[] subAnnotationMenuData(BrAnnotationMenuItem parentAnnotations, List<BrAnnotationMenuItem> brAnnotationMenuItemList){
        ...
        for(BrAnnotationMenuItem brAnnotationMenuItem : brAnnotationMenuItemList){
//          查找子节点
            if(parentAnnotations.getId() == brAnnotationMenuItem.getParentId()){
                AnnotationMenuData subAnnotationMenuData = new AnnotationMenuData();
                subAnnotationMenuData.setCaption(brAnnotationMenuItem.getCaption());

                AnnotationMenuData[] sub = subAnnotationMenuData(brAnnotationMenuItem, brAnnotationMenuItemList);
//              为叶子节点
                if(sub == null){
                   ...
                   annotationRef.setDataName(brAnnotationMenuItem.getName());
    ...
    public Integer subCountByUser(BrAnnotationMenuItem parentAnnotations, List<BrAnnotationMenuItem> brAnnotationMenuItemList, String[] menuItem){
        Integer count = 0;
        for(BrAnnotationMenuItem brAnnotationMenuItem : brAnnotationMenuItemList){
            if(parentAnnotations.getId() == brAnnotationMenuItem.getParentId()) {
                Integer brAnnotationsId = brAnnotationMenuItem.getId();
    ...
    public  AnnotationMenuData[] subAnnotationMenuDataByUser(BrAnnotationMenuItem parentAnnotations, List<BrAnnotationMenuItem> brAnnotationMenuItemList, String[] menuItem){
        //处理叶子结点
        int count  = gisMenuService.getSubNumBrAnnotations(parentAnnotations.getId());
        if(count == 0){
            return null;
        ...
        //判断有多少个子节点在权限表中出现过
        count = subCountByUser(parentAnnotations, brAnnotationMenuItemList, menuItem);
        AnnotationMenuData[] items = new AnnotationMenuData[count];
        int itemsIndex = 0;
        for(BrAnnotationMenuItem brAnnotationMenuItem : brAnnotationMenuItemList){
//          查找子节点
...
//              子节点的ID是否在权限里
                ...
                for(int menuItemIndex = 0; menuItemIndex < menuItem.length; menuItemIndex++){
                    if(brAnnotationsId.equals(Integer.parseInt(menuItem[menuItemIndex]))){
                        ...
                        AnnotationMenuData[] sub = subAnnotationMenuDataByUser(brAnnotationMenuItem, brAnnotationMenuItemList, menuItem);
//                      叶子节点
                        if(sub == null){
                            AnnotationRef annotationRef = new AnnotationRef();
                            AnnotationRefQueryParam annotationRefQueryParam = new AnnotationRefQueryParam();
